...
import { IdTokenConfig, AccessTokenConfig, RedisConfig, LogoutConfig, JwksFetcherConfig, OIDCConfig } from './oidcSectionConfig';
...
test('LogoutConfig should create an object correctly', t => {
  const config = new LogoutConfig({ path: '/logout', redirect_uri: 'http://localhost:3000' });
  const obj = config.toObject();
  t.deepEqual(obj, { path: '/logout', redirect_uri: 'http://localhost:3000' });
});

test('JwksFetcherConfig should create an object correctly', t => {
  const config = new JwksFetcherConfig({ jwks_uri: 'http://localhost/jwks_uri' });
  const obj = config.toObject();
  t.deepEqual(obj, { jwks_uri: 'http://localhost/jwks_uri', periodic_fetch_interval_sec: undefined, skip_verify_peer_cert: undefined });
});
