...
const test = anyTest as TestFn<{ authServiceSecretBuilder: AuthServiceSecretBuilder, testSecret: V1Secret }>;
...
test("AuthServiceSecretBuilder should build an authservice config", async (t) => {
  const authServiceConfig = await t.context.authServiceSecretBuilder.buildAuthServiceConfig(
    [t.context.testSecret]
  )
