...
class RedisConfig {
    ...
    constructor(json?: any) {
        if (!json.server_uri || json.server_uri.length < 1) {
            throw new TypeError("server_uri is required and must be at least 1 character long");
        }
        // Validate the URI
...
export class LogoutConfig {
    ...
    constructor(json: any) {
        if (!json.path || json.path.length < 1) {
            throw new TypeError("path is required and must be at least 1 character long");
        }
        if (!json.redirect_uri || json.redirect_uri.length < 1) {
            throw new TypeError("redirect_uri is required and must be at least 1 character long");
        }
        // Validate the URI
...
export class JwksFetcherConfig {
    ...
    constructor(json: any) {
        ...
        if (json.hasOwnProperty('periodic_fetch_interval_sec')) {
            this.periodic_fetch_interval_sec = json.periodic_fetch_interval_sec;
        }
        if (json.hasOwnProperty('skip_verify_peer_cert')) {
...
export class OIDCConfig {
    ...
    constructor(json: any) {
        if (!json.authorization_uri || json.authorization_uri.length < 1) {
            throw new TypeError("authorization_uri is required and must be at least 1 character long");
        }
        if (!json.token_uri || json.token_uri.length < 1) {
            throw new TypeError("token_uri is required and must be at least 1 character long");
        }
        if (!json.callback_uri || json.callback_uri.length < 1) {
            throw new TypeError("callback_uri is required and must be at least 1 character long");
        }
        if (!json.client_id || json.client_id.length < 1) {
            throw new TypeError("client_id is required and must be at least 1 character long");
        }
        if (!json.client_secret || json.client_secret.length < 1) {
            throw new TypeError("client_secret is required and must be at least 1 character long");
        }
        ...
        if (json.hasOwnProperty('cookie_name_prefix')) {
        ...
        if (json.hasOwnProperty('logout')) {
            this.logout = new LogoutConfig(json.logout);
        }

        if (json.hasOwnProperty('jwks')) {
            this.jwks = json.jwks;
        }

        if (json.hasOwnProperty('jwks_fetcher')) {
            this.jwks_fetcher = new JwksFetcherConfig(json.jwks_fetcher);
        }

        if (json.hasOwnProperty('absolute_session_timeout')) {
            this.absolute_session_timeout = json.absolute_session_timeout;
        }

        if (json.hasOwnProperty('idle_session_timeout')) {
            this.idle_session_timeout = json.idle_session_timeout;
        }

        if (json.hasOwnProperty('trusted_certificate_authority')) {
            this.trusted_certificate_authority = json.trusted_certificate_authority;
        }

        if (json.hasOwnProperty('proxy_uri')) {
            // Validate the URI
            new URL(json.proxy_uri);
            this.proxy_uri = json.proxy_uri;
        }

        if (json.hasOwnProperty('redis_session_store_config')) {
            this.redis_session_store_config = new RedisConfig(json.redis_session_store_config);
        }
