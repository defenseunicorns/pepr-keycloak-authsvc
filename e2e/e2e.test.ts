...
test.serial("E2E Test: Create New Client from Generic Secret", async t => {
  ...
  const labelSecret =
    'kubectl label secret client2 -n keycloak "pepr.dev/keycloak=createclient"';
  ...
  try {
    ...
    t.falsy(
      stderr,
      "kubectl command to create new secret produced no stderr output",
    );
